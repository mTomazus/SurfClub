...
class Trongate_pages extends Trongate {
     ...
     */
    function index(): void {
        $data['page_headline'] = 'articles';
        $data['view_module'] = 'trongate_pages';
        $data['view_file'] = 'trongate_pages_index';
        $this->template('public',$data);
    }    

    function draw_index() {
        $data['view_module'] = 'trongate_pages';
        $sql = 'SELECT * FROM trongate_pages WHERE published=1 ORDER BY date_created DESC';
        $data['articles'] = $this->model->query($sql, 'object');
        ...
        foreach($data['articles'] as $key => $article) {
            $data['articles'][$key]->article_url = BASE_URL.$article_trigger.'/'.$article->url_string;
        }
        $this->view('articles_list', $data);
    }
